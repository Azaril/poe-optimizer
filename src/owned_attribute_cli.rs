...
use poe_optimizer_core::{
    ...
    owned_definitions::{StatDefId, StatDefinition, UnitDefinition},
    owned_schema::{
...
/// Append only explicitly supplied actor statistics and units in registry order.
/// All older consumers retain their typed-stat input contract.
pub(crate) fn augment_definitions(
    ...
    for descriptor in entries {
        let allowed = match &descriptor {
            DefinitionDescriptor::Stat(entry) => matches!(&entry.schema,
                SchemaState::Known(stat) if stat.targets == [RuleEntityKind::Actor]),
            DefinitionDescriptor::Unit(entry) => matches!(&entry.schema, SchemaState::Known(_)),
            _ => false,
        };
        if !allowed {
            return Err(invalid(
                "definitions must be known Actor statistics or units",
            ));
        }
