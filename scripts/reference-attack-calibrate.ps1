...
<#
.SYNOPSIS
Generate independent attack/item/support Windows calibration outputs using PoB's bundled Lua DLLs.
.DESCRIPTION
...
if (-not $OutputDirectory) {
    $OutputDirectory = Join-Path $workspace ("local/reference-calibration/attack-" + [DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss-fff'))
}
...
$harness = Join-Path $PSScriptRoot 'reference-attack-pob.lua'
...
foreach ($name in @('mace-wooden', 'mace-smithing', 'mace-wooden-brutality', 'mace-smithing-brutality')) {
