...
searched jointly with the existing class, ascendancy, passive and normal-weapon choices.
These remain diagnostic profiles. Supporting active skills, general equipment modifiers,
other skills and complete native build evaluation remain unfinished.

## Configuration and calculation

Package schema **4**, semantics **`poe2-native-profiles-v4`**, replaces `mace.brutality`
with a top-level `supports` section. Each record stores its data key, source identities,
family, color, represented level/quality, requirements, required/excluded skill types,
...
reject. Schema-1/2/3 packages require regeneration and review of custom edits.
...
revision and the retained tree artifact are unchanged by this migration.
...
remain reusable under the existing backend/content-identity contract.
...
Problem schema **4** requires `support_loadouts` and explicit `tree_search` budgets.
It retains the 0/1 ordinary and 0/1 ascendancy-point scope. A loadout is an array of data
...
Schema 4 rejects legacy `supports` and `locks.support` fields. Problem schemas 1–3 retain
their existing `none`/`brutality_i` inputs and report layouts; their maximum support count
stays one. New problems produce report schema **5**, with array-valued alternative support
loadouts and data-bound candidate identities.
...
item/active/support requirements. Search rejects illegal candidates before dispatch; a
fully illegal locked domain uses zero calculations. Diagnostic evaluation can inspect such
...
Native Mace profile attachments use media version **2** and include the loadout
and exact configured support records. Tree evidence remains version **2**, with separate
ordinary/ascendancy allocation categories. Strict realization checks bind the exported XML,
selected data/backend identity, support instances and configured effects. The XML companion
retains the dataset identity and trust required to replay custom-data exports.
