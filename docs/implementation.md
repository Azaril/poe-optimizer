...
Current work: **reference item-database readiness**, a confirmed correctness repair in
`runs/reference-readiness-worktree` on branch `codex/reference-readiness`, based on the
published combined item-loading branch. The original loading task now finishes before
caller XML import; focused helper checks and independent calibration refresh pass, with
the public-runtime regression and fresh five-build corpus comparison pass. See [the readiness checkpoint](#reference-item-database-readiness-checkpoint).

Preceding work: **native base flask/charm buff loading**, implemented and locally validated
...
Implemented in `runs/reference-readiness-worktree`, branch `codex/reference-readiness`,
based on published `8781c7c`. This changes optional PoB reference initialization, not the
...
| Publication | Repair is local; combined item-loading branch CI 34383255010 remains separate and pending |
...
Resume: review hosted failures if any, and integrate the validated item-loading and readiness
checkpoints while preserving main's resume edits. The read-only next-slice proposal is
...
nil-preserving requirement semantics. Raw prototypes alone are insufficient; full native
