...
unit `295a` and output `295b` separate attribute components from percentages. Catalyst
percentages and scalar units remain unchanged. Owners `295c`/`2974`/`298c`/`29a4` each retain
23 required raw inputs and Partial eligibility/program gaps. The generic value compiler
uses precision1/display0; no Actor contribution or fourth attribute channel is introduced.
See [the data contract](../data/owned/poe2/3887ae68/item-attribute-inputs/README.md).

All Attributes remains one physical modifier occurrence with one amount; PoB's four
Str/Dex/Int/All records are retained as source witnesses. Only ordinary plus-integer,
no-tag/no-generated-prefix members obtain source proof. Negative/decimal raw recognition,
explicit properties, source eligibility, ordered magnitudes and final contributions stay
separate. Requirements and derived resources are not inferred from the flat family.

The Rust [membership compiler](owned-recipe-membership.md) and optional CLI
`--membership-patch` bind the exact prior registry/schema/rules/routing and supplied V1
extension, expand explicit finite additions, and reuse existing V1/V3 schema/refinement
validation. All 1,756 templates gain four structural members (7,024 insertions), preserving
prior scalar fields, members and Partial closures. The 178,797-byte allocation/rule file
plus 199,551-byte patch replace a 9,225,003-byte explicit extension for this transition.
The final runtime schema is unchanged by compact authoring. Preflight bounds references,
products, exact serialized expansion and work before cloning; existing expanded limits,
no-overwrite publication and no-patch behavior remain. No source lookup or wildcard fallback.

Fresh original modifier counts are **5/22/5/4/11**, **47 total**, preserving all 34 prior
occurrences/rolls. Gains are two Intelligence lines and eleven resistance followers
(six Fire, three Cold, two Lightning). Of twenty direct attribute rows, two admit and
eighteen remain Pending. The 53 resolved display observations and all **110 query rows**
are unchanged. Source layouts remain **5/116**; all 116 owned items, 1,333 allocations,
four Corrupted source markers and whole-build gates remain incomplete. Complete native
originals remain **0/5**. Source/member progress is not actor or whole-build parity.

**Validation:** all 27 scoped Rust tests pass: 19 Import tests (eight new patch tests and
eleven existing V1 extension tests), seven optional PoB tests and the full native CLI
publication chain (300.04s). Source tests include 14,048 fresh full parses across all 1,756
constructed bases and all twenty original attribute rows, with independent numeric,
property/catalyst, duplicate and predecessor contrasts. The CLI checks exact prior facts,
compound occurrence identity, Count-unit arithmetic, missing inputs, immutable publication
and stale patch rejection. Independent read-only reviews found no blocking issue in the
authored data, compiler or CLI. Strict workspace/all-feature/all-target and native CLI lint,
nine-package formatting, five-library WASM compilation, native dependency isolation and
Git whitespace checks pass. Test debug assertions and overflow checks are enabled.

One Unmapped fixture needed its inconsistent empty rule owner removed; one new native test
needed a longer-lived attribution binding. No production fix was needed for either test
issue. Existing Python utilities/tests and protected owned-allocation files are unchanged.
...
The authoritative local package is `runs/owned-item-attributes-01/package`: 10,683 registry
entries, 19 files, 44,914,040 bytes including transition.json. Schema is
`a5ced241f994c45eb43dfa157be21c6e89298c0f635bbd0b8be9cd1b1602db71`;
items is `70a9128f19f72b52225c3ef4cd1b89799d2b9401bfba183c7a7c8a222d9f880b`;
source is `e11bb906d9c5c378673db7474a789a679ed70e63345daa866714d9a798d28ad4`;
rules is `50216ecba46a448b57cf4b438362f168bba79c9d8734277f2ac32f15998edac3`.
The run retains Rust authoring/value policy, publication receipt, native CLI snapshot,
fresh original-01 through -05 normalization and source/import/native validation logs.

**Resume:** review ordinary item-rarity inputs against all five originals: the fresh
inventory has twenty pending rarity rows, thirteen plain, seven without preceding-line
combination, spanning originals1–4. They directly precede four blocked attribute/resistance
rows. These are candidate counts, not promised admissions. Chaos Resistance (seven rows
across2/4/5), local defences and added Lightning attack damage remain separate follow-ups.
All eighteen remaining attribute rows have preceding-line ambiguity, and fifteen also
have an unresolved header. Explicitly model decorated magic base selection before admitting
Baron's Omen Sceptre or its Spirit132 display; do not strip names heuristically. Preserve
the compound occurrence and All witness;
projecting quantities to Actor Integer channels requires explicit quantization, activation
and downstream transformation evidence. Whole-item source order, source property/magnitude
lifecycle, collections and allocations remain gates. Compact authoring does not reduce the
materialized package, so load/runtime representation is a separate measured follow-up.
Socket-configuration and per-metric coverage decisions remain pending before dependent
Core/evaluation changes. T1 records Rust-only project tooling/tests as the end state; new
work is Rust, existing Python stays intact until each utility and its tests migrate together.

**Previous checkpoint (after `936aab6`): Fire and Lightning resistance breadth.**
...
order from ID-sorted record storage. All five unchanged originals normalized through its
CLI paths with 116 items, 195 equipment uses, 478 gems, 81 rewards and 110 query rows.
Original05 retained 22 nominal modifier inputs, two item parameters and eight uses. Drafts
remain Pending. Removed the unused item-slot compile-and-call facade, preserving 13 tests.
411 scoped Rust test executions, strict Clippy, formatting, WASM and PoB/Lua Cargo exclusion
passed; receipts are under `runs/owned-item-transforms-01/` and
`runs/owned-modifier-order-01/`. No full build or performance claim was made.
...
operations are unchanged. No source label map, PoB Item/UI object or source conditional
interpreter enters the evaluator. Direct text conversion requires explicit property facts;
missing facts stay Pending rather than becoming false.

The source grammar extracts ASCII letter/underscore runs, preserving case and decoded
spans. Aliases are explicit. Unknown labels and known labels not consumed by the selected
rule block that line. Empty/no tags establish an empty set only inside this admitted
single-member grammar, not catalyst/header absence. Policy text, input maps, token scans,
variable-length comparisons and map expansion have bounded resource accounting. Review
caught and corrected token grammar, encoder reference accounting and comparison costs;
focused regressions cover each counterexample.

**Real input progress:** the untouched original05 Sapphire Ring now converts nominal
cold25 and all five Boolean properties, preserves the Life input and all eight saved
receiving rows. Fourteen append-only IDs introduce nominal cold/all-elemental families;
existing fixed-value definitions, slots and program bodies retain their meanings. New
nominal rule owners remain Partial with no effective contribution. Catalyst/header
absence, ordinary quality, source encoding and complete scaling still prevent whole-item
completion. Game receiver registries remain empty. All five originals still have **0/5
complete native evaluations**, and all **110** fixed query rows remain required.

The optional pinned PoB component oracle ran31 contrasts. It confirms catalyst default20
versus explicit0, independent ordinary quality, signed truncation and exact scalar
grouping `(100 + quality) / 100`. Existing native arithmetic can represent the latter.
It also distinguishes a fixed-line return-to-unity history quirk from the ranged path,
which recomputes the final value. The [item-scaling design](owned-item-scaling.md) records
that evidence without adopting mutable source history in native evaluation. This is
source component evidence, not a native scaling or full-build parity result.

**Validation:** **162 Rust test executions** pass:107 affected Import tests,27 CLI tests
in each feature mode, and one explicitly executed optional source oracle with31 contrasts.
**29 Python tests** pass:16 import and13 resistance exporter tests. Formatting,
workspace/native-only strict Clippy, five-crate WASM compilation and all six PoB/Lua
runtime package exclusions pass. Read-only review found no remaining blocker after the
three regression fixes. Evidence is under `runs/owned-item-conversion-01/`. All nine
protected files and the clean source pin are unchanged. This is an affected-scope suite;
full workspace tests, changed-build throughput and the final owned-only distribution gate
remain separate.
...
**D1c/D2b composition, binding and offline identity checkpoint validated locally.**

BuildProject composes independent typed character/equipment/allocation/skill/choice presets,
keeping exact occurrence identities and checking complete inventory unions before selection.
The owned codec and check-owned-input support projects. Shared record validation avoids
constructing fake characters or revalidating each possible preset combination.

The portable definition binder checks concrete authored values, quality, roles, exact provider
paths, action selections and required choices against injected schemas. Reports bind full
request/package identities and retain ordered query rows. Invalid inputs, unresolved schema,
unavailable historical selectors and generated targets pending resolution remain distinct.
The thin bind-owned-input command accepts only a complete owned request plus caller schema.
No UI defaults, source names, source programs, PoB runtime or calculation are used on this path.

Offline owned-ID registry/mapping artifacts now live in Import. Durable counters/tombstones,
exact typed targets, explicit aliases and source/policy/registry/schema bindings support a
future catalog converter without making external names runtime identity. Identity mapping to
an Unmapped schema does not imply known behavior. Project composition, schema binding and
mapping are prerequisites; they are not the full D1/D2 exit or new numerical coverage.

Independent review found a mapping owner/socket contradiction hole; the checkpoint includes
its consistency checks and regressions. Actual obsolete code removed here is the unused
ValidatedMaceWeapon::is_legacy_normal_payload classifier and classification-only assertions.
Header spelling/rejection, quality values, modifier capture and equip-level laws remain.
The broader active Spark/Mace dependency closure still requires semantic evaluator migration.

Final local validation passes **229 selected test executions, zero failures**: all Core
targets (157), Data schema tests (11), three owned CLI targets under default features (13)
and native-only features (13), offline mapping tests (16), retained mace-item tests (10) and
broader equipment tests (9). Strict workspace/all-target and native-only Clippy, five portable
crate WASM checks, formatting and the native-only normal dependency check pass. That dependency
tree contains no PoB/Lua runtime. All 900 checked local documentation link occurrences resolve.
The original builds, legacy schema-40 package, Cargo.lock and clean PoB pin are unchanged.

`runs/owned-binding-implementation-01/validated-checkpoint.json` joins all twelve producing
commands, stable source/input guards, review and documentation checks. The final sequence is
`validation-01`. Development checks first caught colliding test helper names, an oversized
error payload and two collapsible registry conditionals; these were corrected without lint
suppressions, dropped numerical expectations or new profiles. All validation runtimes are
terminal. Transient approval-service capacity rejections recovered before publication.

The preceding pushed `9aff988` [CI run 34822087317](https://github.com/Azaril/poe-optimizer/actions/runs/34822087317)
had four successful jobs and four running at 09:11:56 UTC on 2026-09-14, with no failed steps.
That observation is not a full-run or later-checkpoint result. Current implementation details
are in [owned composition and binding](owned-binding.md).

**D1d owned draft/selection checkpoint validated locally.**

Portable Core now stores partial semantic sessions with independent build/scenario/query
presets, stable unresolved issue IDs, component-partial selectors and explicit collection
completion. Validation uses the existing structural leaves, checking known facts and complete
candidate values without installing candidates or manufacturing missing records. Unresolved
source meaning stays outside complete evaluation; no Lua/source/UI program is introduced.

Explicit selected finalization follows the selected rows and backing item/gem records through
existing complete project/composition/request constructors. Inactive alternatives and global
registry uncertainty stay saved; selected membership/fields must be complete. Disabled or
off-loadout selected rows are not silently pruned. Pending output preserves every ordered query
row. Complete output binds exact draft/selection/request identities and makes no schema,
legality, inventory-availability or numerical claim.

The separate bounded draft codec and thin check-owned-draft CLI serve both imports and direct
authoring. Import's injected lexical codec handles exact decimal integer bounds, explicit
whitespace/token grammar, units and rational quantity scales. Source precedence/role policies
and all-five normalization remain next work; the codec has no game-specific field names.
Independent review identified and fixed a pending-parent owner-family hole before publication.
The validator rejects impossible declaration families without guessing the pending exact
definition; known and candidate slots have regression coverage.

Final local validation passes **274 selected test executions, zero failures**: all Core
cases (191), Data schema tests (11), Import mapping/value tests (28), and four owned CLI
suites with default features (22) and without PoB (22). Strict workspace/all-target and
native-only Clippy, CORE/DATA/ENGINE/IMPORT/NATIVE WASM compilation, formatting and the
native-only normal dependency check pass. No PoB/Lua runtime appears in that tree. All 922
checked local documentation link occurrences resolve. The original inputs, legacy schema-40
package, Cargo.lock and clean pinned PoB checkout are unchanged.

runs/owned-normalization-01/validated-checkpoint.json joins all ten final producing commands,
source/input guards, independent reviews and documentation checks. The final sequence is
validation-02; validation-01 caught an oversized pending-result enum. Boxing its selection
preserves the wire format and all tests/assertions. Earlier development checks caught a
macro tokenization error and unused import; they were corrected without dropped tests,
lint suppression or new profiles. All validation runtimes are terminal. Transient approval
service capacity failures recovered before publication.

For preceding commit 5ce8a08, [CI run 34827697718](https://github.com/Azaril/poe-optimizer/actions/runs/34827697718)
had both platform validation jobs and both native suites passing, with four broader suites
still running and no failed steps at 09:50:09 UTC on 2026-09-14. This is not a full-run or
later-commit CI result; existing Windows/Linux CI includes the new tests automatically.

See [owned drafts](owned-drafts.md). This is not full D1/D2 completion. No numerical consumer
was retired in this checkpoint; active Spark/Mace consumers remain named in the retirement
inventory until shared numerical replacements preserve their meaningful tests. No third
profile, source VM expansion, changed original fixture or new tolerance is permitted.

**Next:** normalize all five originals through the delivered draft boundary using the
import occurrence/value policy, durable mapping registry and adapter audit. Develop
revisioned pending repair/edit/adoption alongside that work; an editor is not a prerequisite
for collecting and representing all five builds. Keep source occurrences/explanations in Import sidecars; do
not reuse legacy selected-view admission or infer configuration values through f64 first.
The bounded policy proposal is runs/owned-normalization-01/policy-proposal.md; unresolved
source semantics remain explicit. Then compile representative local-item, conditional,
support and grant effects into owned rules and develop shared player/minion resolution.
Remove replaced legacy numerical callers while preserving independent numerical tests.
Complete native coverage remains **0/5** and legacy search-build still uses closed profiles.
No user decision is needed for this agreed structural work.
...
**CI schema assertion repair: local validation complete.** The completed Ubuntu and
Windows jobs in [run `34653246253`](https://github.com/Azaril/poe-optimizer/actions/runs/34653246253)
(head `c69810d`) exposed a separate stale expectation in
`bundled_catalog_retains_complete_declarations_and_constructed_rows`: the package reports
schema 29 while the test still asserted 28. The same assertion remained at published
`7a9c40a`. The test now compares against `game_data::SCHEMA_VERSION`, preserving all
catalog counts, capability and identity-resolution checks. Dedicated package tests still
pin schema 29 and its semantics identifier. A workspace audit of 83 schema assertions
found no other stale expectations. All 11 skill-identity tests and strict scoped Clippy
pass. Neither the package nor production code changes for this repair. Authenticated
...
imports cannot fabricate allocation observations. A required source-runtime profile
records the pinned VM/build conventions being emulated, independent of the Rust host.
The initial admission is a unique empty, zero-allocation TNEW constructor. Multiple or
nonempty constructors, constant templates and nested constructor sites remain explicit
follow-up work. No parser wire schema or bundled data change is needed.

Native sessions privately track array-only allocation, occupancy and integer rehash
transitions for admitted constructors. Nil-to-absent writes and deletions preserve the
source-required allocation history; unsupported hash transitions invalidate the layout
without preventing explicit-key writes. Snapshots do not recreate that history. All
creation, mutation, traversal and result allocations use the existing cumulative budgets.
The compiler shares immutable metadata and looks up constructor sites by callback/range;
worker sessions retain independent mutable state without Lua or subprocesses.

Raw length, the length operator and warmed JIT hints remain separate contracts. Default
unpack and append reject a simulated layout if raw length can disagree with a valid hinted
length; append rejects before writing. Explicit unpack ranges bypass default length.
An independent review found that table.insert needs this same guard as unpack; the fix,
its no-write regression and all affected final checks are included. No hint history is
inferred from a single matching interpreter sample.

The actual full `Common.copyTable` now passes **220 deep/shallow copies**, including
**40 actual cache rows**, across all five originals. The suite compares **50 observed-input
default-unpack cases**, **30 copied default-unpack cases**, **50 explicit copied ranges**,
**20 source errors**, and five continuing value replacements. It requires **20 copied
layout frontiers** and five post-structural-write observation frontiers to remain
unsupported. A companion negative control uses the same logical source without allocation
evidence and verifies that it cannot silently admit a copied sparse default length.
Source-required nested alias splitting, shallow aliases and function identity remain intact.

Exact-function warmed comparisons cover **40 copy vectors / 5,120 calls** and **30 unpack
vectors / 3,840 calls**, with completed traces containing the actual target function.
Nine separate length-history probes retain cold and post-seed result packs; those
observations do not establish universal hint equivalence. The continuing original callback
sequence remains **470/637** at `modLib.parseMod`, with the original boss/preset, rounding,
StripEscapes, setSource and AddMod gates retained. Complete native evaluation is still
**0/5 originals**. This checkpoint removes a parser dependency; it does not complete the
public parser or the real-build evaluator.

Validation passes **76 data**, **83 observer/lowerer**, **154 engine**, **20 source
integration**, **16 default CLI** and **14 native-only CLI** test/configuration cases:
**363 focused cases**, plus **11 skill-identity cases** for the separate CI repair.
Repeated supporting checks are not counted twice. Two fresh exports reproduce all 29
package sections. Strict workspace/native Clippy, formatting, WebAssembly library
compilation and native dependency isolation pass. The final source run contains
**58 fresh artifacts** and verifies **116 source/helper hashes** stayed unchanged.
Independent data/observer/engine reviews are closed.

Evidence is bound by `runs/r2m-integration-validation.json`; publication is tracked in
`runs/r2m-publication.json`. The earlier source run passed its tests but was superseded
because the append guard changed during that run; default CLI validation was also rerun
afterward. Native-only CLI validation began after the final production correction and
is retained with source/log provenance. Original user inputs, reference fixtures, lockfile,
source pin and schema-29 package remain unchanged. Numerical execution is local Windows
x64; WebAssembly validation is compile-only. The CI snapshot in
`runs/r2m-ci-final-review.json` records published `7a9c40a` still in Test on both platforms,
two newly reviewed schema failures on older `c69810d`, and the previously fixed older
benchmark failure. This is not hosted pass evidence; check the next exact-head run.

**Resume point: R2n complete original public parser session.** Keep one milestone with a
fixed finish condition: the real public parser from an uncached line through its returned,
writable modifier graph, retaining startup history, cache aliases, dictionary mutations
and live capture cells. Prefer one source owner and ProgramSession for parser and consumer.
The current stateless parser facade is not an equivalent inner operation: it resets
accounting and duplicates retry/copy behavior. Any reused native inner kernel must borrow
the session's live state and cumulative budget with an authenticated source binding.

The connected dependencies are multiple/nonempty constructors and templates; mixed-key
traversal and deletion history; mixed assignment and source-created lexical closures;
original string lower/find/sub, bit operations and callable iterator identities; and the
full initialized parser/cache/dictionary graph. Preserve explicit frontiers for unmodeled
layout and optional logging/environment writes. These are parallel implementation tasks
within the parser milestone, rather than separate claims of parser completion. The
[parser-session design](parser-sessions.md) remains the contract; detailed source locations
and API considerations are in `runs/r2m-parser-session-next-milestone.md`.

Acceptance includes preloads, uncached success and retry truthiness, cached no-match,
case-sensitive keys, cache hits after dictionary mutation, eviction/reparse, DOUBLED
followed by another uncached line, produced closure cells, failure write prefixes,
independent writable copies, worker/session isolation and cumulative budget retention.
Then run the real multiline quest/custom consumers and complete default/saved activation
on Twister and Skeletal Sniper together, retaining all five originals and mapping variants.
A cache-hit replay or wrapper linked to an invented result callback does not satisfy this
gate. No new design-direction decision is needed for these accepted requirements.

**Previous checkpoint: R2l mutable traversal and original copy/unpack consumers.**
...
**Historical R2l follow-up: native constructor/layout and full public parser sessions.**
