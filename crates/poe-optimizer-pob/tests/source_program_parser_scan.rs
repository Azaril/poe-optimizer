...
fn observe(
    ...
    let extra_callbacks = if with_closures {
        flag_helpers::NAMES
            .iter()
            .map(|name| {
                (
                    format!("flag.{name}"),
                    lua.globals().raw_get(*name).unwrap(),
                )
            })
            .collect()
    } else {
    ...
    let parser_capture::CapturedParser {
        observed,
        lowered,
        inventory: mut report,
    } = parser_capture::capture(
        lua,
        primitives,
        scan,
        parser,
    ...
    let copy_table: Function = lua.globals().raw_get("copyTable").unwrap();
