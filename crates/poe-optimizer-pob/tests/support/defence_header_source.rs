...
impl Source {
    pub fn new() -> Self {
        ...
        oracle
            .lua
            .globals()
            .set("defence_snapshot", snapshot.clone())
            .unwrap();
        oracle
            .lua
            .load(include_str!("defence_header_observer.lua"))
            .set_name("@test-only-defence-header-observer")
            .exec()
            .unwrap();
        Self { oracle, snapshot }
    }
    pub fn clear(&self) {
        for key in ["defence_stages", "defence_calls"] {
            self.oracle
                .lua
                .globals()
