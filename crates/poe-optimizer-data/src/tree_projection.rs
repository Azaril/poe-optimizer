...
impl AuthenticatedTreeSnapshot {
    ...
    /// Host-side trust constructor after a complete, supervised source extraction.
    /// The caller must obtain `expected_sha256` from that trusted producer boundary;
    /// a digest supplied alongside arbitrary external data does not authenticate it.
    pub fn from_trusted_extraction(
        snapshot: TreeDataSnapshot,
        expected_sha256: &str,
    ) -> Result<Self> {
        let mut value = Self::from_trusted_digest(snapshot, expected_sha256)?;
        value.provenance = AuthenticationProvenance::FreshPinnedExtraction;
        Ok(value)
    }
...
fn validate_node_metadata(snapshot: &TreeDataSnapshot, node: &TreeNode) -> Result<()> {
    validate_node_metadata_for_catalog(&snapshot.classes, &snapshot.ascendancies, node)
}
pub(crate) fn validate_node_metadata_for_catalog(
    classes: &BTreeMap<u32, crate::tree_data::TreeClass>,
    ascendancies: &BTreeMap<String, crate::tree_data::TreeAscendancy>,
    node: &TreeNode,
) -> Result<()> {
    ...
    for (selector, option) in &node.automatic_overrides {
        let class = classes.values().find(|class| class.name == *selector);
        let ascendancy = ascendancies.values().find(|asc| asc.name == *selector);
        ...
        crate::tree_data::effective_source_node(
            classes
                .get(&class_id)
                .ok_or_else(|| unsupported(node.id, "missing effective class"))?,
            ascendancy_id.and_then(|id| ascendancies.get(id)),
            node,
        )
        .map_err(|error| unsupported(node.id, format!("invalid effective override: {error}")))?;
    }
    Ok(())
}
