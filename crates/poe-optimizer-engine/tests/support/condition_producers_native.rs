...
use poe_optimizer_engine::{
    ...
    modifiers::{
        ModifierDatabase, ModifierInput, ModifierKind, ModifierValue, NumericKind, QueryContext,
        SumKind, TaggedModifierInput,
    },
...
pub fn query(input: &Value) -> Result<Observed, String> {
    ...
    match input["query"]["kind"].as_str().unwrap() {
        ...
        "sum" => {
            ...
            let database = ModifierDatabase::try_new_tagged(layers).map_err(|e| e.to_string())?;
            let names: Vec<_> = input["query"]["names"]
                .as_array()
                .unwrap()
                .iter()
