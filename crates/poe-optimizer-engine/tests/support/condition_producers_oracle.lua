...
function condition_oracle_prepare(input)
	...
	if query.kind == 'flag' then
		return function() return db:Flag(cfg, unpack(query.names)) end, db, cfg
...
-- Each actual source consumer gets an independent monomorphic call loop. A
-- completed trace for a nested consumer does not prove its vararg wrapper was
-- compiled, so retain a direct Flag control in addition to the original chain.
...
local function warm_flag(db, cfg, query, repetitions)
	local value
	for _ = 1, repetitions do value = db:Flag(cfg, query.names[1]) end
	return value
end
